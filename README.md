...
Este trabalho mostrou como a algortimos de clusterização podem ser uma ferramenta de apoio no processo de identificação dos períodos transientes nos dados de um equipamento. Embora os resultados das clusterização tenha espaço bastante espaço para melhoria, o processo de obtenção dos clusters e montagem do grafo de transição entre os estados de operação tem potencial de ajudar o especialista na difícil e cara tarefa de rotular os dados. 

Existem algumas oportunidades de trabalhos futuros dando continuidade a este estudo. A primeira e mais natural é continuar testes com outros algoritmos de clusterização para melhorar o resultado até aqui obtido. Métodos Evoltuivos de clusterização tem sido estudados e poderia ser uma abordagem interessante. Como novas condições de operação podem ocorrer mesmo após muito tempo de operação de um equipamento, este tipo de algoritmo tem potencial de resultados melhores.

O grafo gerado como resultado da clusterização tem poder de representar a transição entre as condições operacionais do equipamento. Esse grafo tem potencial de ser utilizado como entrada para outros algortitmos de aprendizado de máquina. Relacionar as condições operacionais de um equipamento com dados manutenções realizadas, trocas de peças, comentários das equipes de manutenção, estoque de sobressalentes, ou qualquer outra informação sobre aquele indivíduo tem potencial de ser o ponto de partida para várias outras análises que atualmente parecem extremamente distanted, como, por exemplo,  ressuprimento de sobressalentes e abertura de ordens de manutenção automáticas baseadas na condição do equipamento. 
